...
> **Status:** v0.22.x — Compute-heavy genes stopped failing on our own fuel constant. An evaluation run that dies of fuel alone now retries up a fuel ladder — wall clock and memory stay fixed, and the fuel a gene actually burns still lands in `resource_cost`, so nothing gets cheaper by being expensive — and the public run ledger records a `failure_kind`, so a reader auditing a score can tell an evaluation-resource ceiling from a gene defect. `rotifer test` climbs the same ladder, so admission and test agree about what a compute-heavy gene is. The evolve-life family, kept out of admission by the old flat budget, now passes 3/3 and is back on the board at 0.3.0 — with the particle family, all six genes this project invalidated have walked the invalidated → repaired → returned path. Hybrid genes gain a first host-function preview (`rotifer.net` / `rotifer.kv` / `rotifer.env`), nonstandard and deny-by-default: nothing is linked unless the deployer grants it and the phenotype declares it, and a `Native` module importing any of them is refused at compile time and at run time. Experimental P2P gene propagation (`rotifer network`) remains off by default (`enabled: false`) and ships with no default bootstrap peer, so you pass one explicitly (`rotifer network start -b <multiaddr>`) — there is no public network to join yet. See [CHANGELOG.md](CHANGELOG.md) for full release history. Automatic peer discovery, internet-wide reach, and L4 Collective Immunity are later milestones — see [Implementation Status](#implementation-status) below.
