...
> **Status:** v0.25.x — a correctness release for the number the Arena ranks on. F(g) was dividing by the efficiency scores instead of multiplying them: `L_score` and `Cost_score` are already inverted — lower latency and lower cost sit nearer 1 — so the spec's penalty had become a reward. Fitness rose with latency, everything past roughly a second pinned at the old `min(1.0)` cap (six leaderboard genes tied at 1.000, and a hello-world measured at 8.4s scored a perfect 1.0000), and ranking was selecting against performance. The terms are now multiplied and the cap is gone — dividing was what made the value unbounded; multiplied, the formula is bounded by its own factors — with one shared definition behind the CLI and its tests and the same fix in the Rust core. **Expect scores and rankings to move.** This is half of the revised form: latency and cost are still measured against fixed 1000ms / 10000-fuel constants rather than the season median for a gene's own domain and fidelity, which needs a pipeline that does not exist yet. Those constants are marked provisional and are far out of scale — the bundled Native corpus burns 1.2M–83M fuel against a 10k scale, landing every measured gene near F(g)=0.003 — so the tau gate stays suspended while `hasCompleteFitnessInputs` is false rather than rejecting every gene against a threshold it was not calibrated for; V(g) still hard-gates, the CLI prints F(g) with a warning, and the gate returns with the calibrated scale. Also here: wasmtime 47.0.4 closes a high-severity filesystem sandbox escape, `rotifer wrap --from-clawhub` follows the 307 ClawHub's download endpoint now returns instead of piping the redirect body into `unzip`, and `rotifer self-update` actually updates `@rotifer/mcp-server` — it had been comparing that package against the CLI's own version number and silently skipping it. The anonymous usage signal is unchanged: no identity, on by default, answering only "was any machine active, from which channel, how many times", with every field and every way to turn it off (`rotifer telemetry off`, `ROTIFER_TELEMETRY=0`, or the cross-tool `DO_NOT_TRACK=1`) documented in [TELEMETRY.md](TELEMETRY.md). See [CHANGELOG.md](CHANGELOG.md) for full release history. Automatic peer discovery, internet-wide reach, and L4 Collective Immunity are later milestones — see [Implementation Status](#implementation-status) below.
