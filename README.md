...
> **Status:** v0.21.x — What a profile says and what a fresh workspace can run both got honest. A developer's steward's note now carries translations, each pinned to the exact English text it translates — edit the note and a stale translation stops rendering instead of silently misrepresenting you (rotifer.dev serves the Chinese reading today). The L0 gate now judges a gene's provenance by who wrote its `.cloud-manifest.json` — `install` stamps `installed_at`, `publish` stamps `published_at` — instead of treating any manifest as foreign, so on machines without the native addon your own and bundled genes run (with a warning) while installed ones are still refused, and anything unreadable stays refused. `rotifer init` now ships `gene-health-scanner`, the gene its own recommended first command depends on, and the stated Node floor (>= 22.18) matches the one the CLI already had in practice. Experimental P2P gene propagation (`rotifer network`) remains off by default (`enabled: false`) and ships with no default bootstrap peer, so you pass one explicitly (`rotifer network start -b <multiaddr>`) — there is no public network to join yet. See [CHANGELOG.md](CHANGELOG.md) for full release history. Automatic peer discovery, internet-wide reach, and L4 Collective Immunity are later milestones — see [Implementation Status](#implementation-status) below.
