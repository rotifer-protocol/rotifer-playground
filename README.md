...
> **Status:** v0.20.x — The authoring Skills get public source, on top of v0.19's Arena integrity work. `rotifer-guide`, `rotifer-agent` and `rotifer-arena` have been installable from ClawHub since April with their source sitting in a repository nobody could read; they now live in [`skills/`](skills/) with CI over them, and the Gene development manual all three route a reader to is published alongside them instead of those routes ending in nothing. Every agent host that installs the plugin bundle now calls it by the same name. Underneath, a score still has to say how it was earned: the invalidation criteria ship as code anyone can run, every entry carries provenance, the leaderboard separates what it has measured from what it has only estimated, and an author can take a version down (`rotifer unpublish`) with the takedown on the public record. Experimental P2P gene propagation (`rotifer network`) remains off by default (`enabled: false`) and ships with no default bootstrap peer, so you pass one explicitly (`rotifer network start -b <multiaddr>`) — there is no public network to join yet. See [CHANGELOG.md](CHANGELOG.md) for full release history. Automatic peer discovery, internet-wide reach, and L4 Collective Immunity are later milestones — see [Implementation Status](#implementation-status) below.
