...
> **状态：** v0.25.x——一次针对 Arena 排名所依据的那个数字的修正。F(g) 此前把效率项**除**进公式而不是相乘：`L_score` 与 `Cost_score` 本身已是反向指标——延迟越低、成本越低，取值越接近 1——所以规范里的惩罚项变成了奖励。适应度随延迟上升，超过约一秒的基因全部顶在旧的 `min(1.0)` 上限（排行榜上六个基因并列 1.000，一个实测 8.4 秒的 hello-world 拿到满分 1.0000），排名实际在筛选性能更差的基因。现在两项改为相乘，上限一并移除——当初正是相除让取值无界；相乘之后公式由自身各因子天然有界——CLI 与其测试共用同一份定义，Rust 核心也落了同一处修复。**既有分数与排名会发生变化。** 这只是修订形态的一半：延迟与成本仍以固定的 1000ms / 10000 fuel 为尺度衡量，而非该基因所属领域与保真度在本赛季的中位数——后者需要一条尚不存在的管线。这两个常数已标注为暂定值，且严重不合尺度——内置的 Native 基因集消耗 1.2M–83M fuel，对照的却是 10k 的尺度，导致每个被测基因都落在 F(g)=0.003 附近——因此在 `hasCompleteFitnessInputs` 为 false 期间，tau 门保持暂停，而不是拿一个未经校准的阈值去否掉所有基因；V(g) 仍为硬门，CLI 会带警告地打印 F(g)，待尺度校准完成后该门恢复。本版另含：wasmtime 升级至 47.0.4，修复一个高危文件系统沙箱逃逸；`rotifer wrap --from-clawhub` 现在会跟随 ClawHub 下载端点改用的 307，而不再把重定向响应体直接送进 `unzip`；`rotifer self-update` 现在真的能升级 `@rotifer/mcp-server`——此前它拿 CLI 自身的版本号去比对该包，于是每次都被静默跳过。匿名使用信号维持不变：不含身份标识，默认开启，只回答「是否有机器活跃、来自哪个渠道、发生了多少次」；全部字段与所有关闭方式（`rotifer telemetry off`、`ROTIFER_TELEMETRY=0`，或跨工具通用的 `DO_NOT_TRACK=1`）均记录在 [TELEMETRY.md](TELEMETRY.md)。详细版本历史请参见 [CHANGELOG.md](CHANGELOG.md)。自动节点发现、跨公网连接与 L4 集体免疫是后续里程碑——详见下方[实现状态](#实现状态)。
